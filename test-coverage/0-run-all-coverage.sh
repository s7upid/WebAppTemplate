#!/usr/bin/env bash
# ============================================
# 0. Run All Coverage (Master Script)
# ============================================
# Runs all coverage scripts in sequence:
#   1. Backend (.NET) Coverage
#   2. Frontend Jest Coverage
#   3. Frontend Cypress Coverage
#   4. Extract Results
...
echo "----------------------------------------"
if bash "$SCRIPT_DIR/1-run-be-coverage.sh"; then
...
echo "----------------------------------------"
if bash "$SCRIPT_DIR/2-run-fe-jest-coverage.sh"; then
...
# Step 3: Cypress Coverage
echo "[3/5] Running Frontend Cypress Coverage..."
echo "----------------------------------------"
if bash "$SCRIPT_DIR/3-run-fe-cypress-coverage.sh"; then
...
if node 4-extract-results.js; then
...
echo "----------------------------------------"
if node 5-update-readme-badges.js; then
